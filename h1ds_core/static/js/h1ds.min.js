...
function PlotContainer(g,m,q){this.id=g;this.plotSpacing=5;this.plotTypes={raw:this.plotLine,spectrogram:this.plotSpectrogram,powerspectrum:this.plotLine,raw2d:this.plotSpectrogram};this.svg=d3.select(g).append("svg:svg").attr("width",$(g).width());this.data_colours="#A11D20 #662C91 #1859A9 #008C47 #ED2D2E #010101".split(" ");this.plotGrid=this.setPlotGrid(m,q);this.url_cache={};this.data_ids={};this.dataPadding=0.05;this.xAxisPadding=40;this.yAxisPadding=60}
PlotContainer.prototype.plotLine=function(g){var m=g.datum().plot.flip;g.append("path").classed("path-filled",function(g){return g.data.is_minmax}).style("stroke",function(g,c){return g.data.colour}).style("fill",function(g,c){return g.data.colour}).attr("d",function(g,c){if(g.data.is_minmax){var a=fillPlot(g.data),e=d3.svg.line().x(function(a){return g.plot.x(a[m?1:0])}).y(function(a){return g.plot.y(a[m?0:1])});return e(a)}e=d3.svg.line().x(function(a,c){return g.plot.x(m?a:g.data.dim[c])}).y(function(a,
c){return g.plot.y(m?g.data.dim[c]:a)});return e(g.data.data)})};PlotContainer.prototype.plotRaw2D=function(g){};
PlotContainer.prototype.plotSpectrogram=function(g){var m=[];d3.scale.linear().domain([0,1]).range(["blue","red"]);d3.scale.linear().domain([0,1]).range([0,1]);var q=0;data=g.datum();for(var c=data.plot.x(data.data.dim[0][1])-data.plot.x(data.data.dim[0][0]),a=data.plot.y(data.data.dim[1][0])-data.plot.y(data.data.dim[1][1]),e=data.plot.x(data.data.dim[0][0]),f=data.plot.y(data.data.dim[1][0]),p=0;p<data.data.dim[0].length-1;p++)for(var t=0;t<data.data.dim[1].length-1;t++)0<data.data.data[p][t]&&
(m.push({xi:p,yi:t}),data.data.data[p][t]>q&&(q=data.data.data[p][t]));g.selectAll("rect").data(m).enter().append("rect").attr("x",function(a,f){return e+a.xi*c}).attr("y",function(c,e){return f-c.yi*a}).attr("width",c).attr("height",a).style("fill",function(a,c){return data.data.colour}).style("fill-opacity",function(a,c){return data.data.data[a.xi][a.yi]/q})};PlotContainer.prototype.plotPowerSpectrum=function(g){};
PlotContainer.prototype.setPlot=function(g,m,q){for(var c in m)this.plotGrid[g][c]=m[c];"undefined"!==typeof q&&q&&this.updateDisplay()};
PlotContainer.prototype.setPlotGrid=function(g,m){for(var q=d3.sum(m),c=[],a=this.svg.attr("width"),e=d3.min([a/1.618,0.9*$(window).height()]),f=d3.sum(g),p=[],t=0;t<m.length;t++)c[t]=a*m[t]/q;for(t=0;t<g.length;t++)p[t]=e*g[t]/f;q=[];for(t=f=e=a=0;t<g.length;t++){for(var N=f=0;N<c.length;N++)q[a]={translate:[f,e],width:c[N],height:p[t],data_ids:[],data:[],plotType:"",flip:!1,bindAxis:[-1,-1]},f+=c[N],a++;e+=p[t]}this.svg.attr("height",e);$(this.id).height(e);return q};
PlotContainer.prototype.addData=function(g,m){this.data_ids[g]={uri:m,colour:this.data_colours.pop()}};PlotContainer.prototype.addDataToPlot=function(g,m,q){q="undefined"!==typeof q?q:!0;this.plotGrid[m].data_ids.push(g);q&&this.updateDisplay()};ProcessH1DSHeaders=function(g){g=g.split("\n");for(var m={},q=0;q<g.length;q++){var c=g[q].split(":");2===c.length&&c[0].match("^X-H1DS")&&(m[c[0]]=$.trim(c[1]))}return m};
...
function str2ab(g){for(var m=new ArrayBuffer(g.length),q=new Uint8Array(m),c=0,a=g.length;c<a;c++)q[c]=g.charCodeAt(c);return m}PlotContainer.prototype.loadURL=function(g,m){var q=this;if(m){var c=$.ajax({url:g,async:!1}),a=ProcessH1DSHeaders(c.getAllResponseHeaders());c.done(function(c){data=ProcessBinaryData(c,a);q.url_cache[g]=data})}else $.ajax({url:g,dataType:"json",async:!1}).done(function(a){a.is_minmax=isMinMaxPlot(a);a.n_dim=2===a.dim.length?2:1;q.url_cache[g]=a})};
PlotContainer.prototype.getData=function(g,m){var q={};switch(m){case "raw":var q=getRawUri(this.data_ids[g].uri,this.svg.attr("width")),c=q.renderUri();this.loadURL(c,q.isBinary());q=this.url_cache[c];break;case "spectrogram":q=getSpectrogramUri(this.data_ids[g].uri);c=q.renderUri();this.loadURL(c,q.isBinary());q=this.url_cache[c];break;case "powerspectrum":q=getPowerspectrumUri(this.data_ids[g].uri,this.svg.attr("width"));c=q.renderUri();this.loadURL(c,q.isBinary());q=this.url_cache[c];break;default:this.loadURL(this.data_ids[g].uri,
!1),q=this.url_cache[this.data_ids[g].uri]}return q};
...
a){d3.select(this).call(g.plotTypes[c.plot.plotType])});m.exit().remove()};PlotContainer.prototype.linkAxes=function(g){var m=0<=this.plotGrid[g].bindAxis[0]&&this.plotGrid[g].bindAxis[0]!==g?this.plotGrid[g].bindAxis[0]:g,q=0<=this.plotGrid[g].bindAxis[1]&&this.plotGrid[g].bindAxis[1]!==g?this.plotGrid[g].bindAxis[1]:g;m!==g&&this.plotGrid[g].x.domain(this.plotGrid[m].x.domain());q!==g&&this.plotGrid[g].y.domain(this.plotGrid[q].y.domain())};
PlotContainer.prototype.updatePlot=function(g){var m=this.plotGrid[g].flip,q=[this.yAxisPadding,this.plotGrid[g].width],c=[this.plotGrid[g].height-this.xAxisPadding,0],a=[d3.min(this.plotGrid[g].data,function(a,c){return 1===a.n_dim?d3.min(a.dim):d3.min(a.dim[0])}),d3.max(this.plotGrid[g].data,function(a,c){return 1===a.n_dim?d3.max(a.dim):d3.max(a.dim[0])})],a=d3.scale.linear().range(m?c:q).domain(a);this.plotGrid[g].flip?this.plotGrid[g].y=a:this.plotGrid[g].x=a;var a=[d3.min(this.plotGrid[g].data,
...
this.responseText.split("\n");$.getJSON("/_/url_for_shot",{input_path:window.location.toString(),shot:g[g.length-2]},function(g){window.location=g.new_url})}}function toggleTrackLatestShot(){"true"===$.cookie("shotTracking")?turnOffShotTracker():turnOnShotTracker();return!1}function plotScalar(g,m){return 0}function plot1DimArray(g,m){var q=new PlotContainer("#"+g,[500],[1]);q.addData("default",m);q.setPlot(0,{plotType:"raw"});q.addDataToPlot("default",0,!0)}
function plot2DimArray(g,m){var q=new PlotContainer("#"+g,[500],[1]);q.addData("default",m);q.setPlot(0,{plotType:"raw2d"});q.addDataToPlot("default",0,!0)}function getPlotFunction(g,m){switch(m){case "0":return plotScalar;case "1":return plot1DimArray;case "2":return plot2DimArray}}
