...
!0)}}})})(jQuery);(function(a){a.vakata.context={hide_on_mouseleave:!1,cnt:a("<div id='vakata-contextmenu' />"),vis:!1,tgt:!1,par:!1,func:!1,data:!1,rtl:!1,show:function(c,e,g,f,l,n,U){a.vakata.context.rtl=!!U;if(c=a.vakata.context.parse(c)){a.vakata.context.vis=!0;a.vakata.context.tgt=e;a.vakata.context.par=n||e||null;a.vakata.context.data=l||null;a.vakata.context.cnt.html(c).css({visibility:"hidden",display:"block",left:0,top:0});if(a.vakata.context.hide_on_mouseleave)a.vakata.context.cnt.one("mouseleave",
...
arguments.length?t&&t.on:(t&&(t.on=null,r=r.slice(0,e=r.indexOf(t)).concat(r.slice(e+1)),c.remove(s)),a&&r.push(c.set(s,{on:a})),d)},s}function v(){m.event.stopPropagation();m.event.preventDefault()}function R(){for(var d,s=m.event;d=s.sourceEvent;)s=d;return s}function U(d,s){function a(){d.on(s,null)}d.on(s,function(){v();a()},!0);setTimeout(a,0)}function y(d){for(var s=new g,a=0,c=arguments.length;++a<c;)s[arguments[a]]=q(s);return s.of=function(a,r){return function(c){try{var J=c.sourceEvent=
...
J/u;u=-p*r/u;r=zd(e,c);e=rb(e,J);c=rb(c,u);jc(e,c);c=qb(e,r);J=qb(r,r);u=c*c-J*(qb(e,e)-1);if(!(0>u)){var t=Math.sqrt(u),u=rb(r,(-c-t)/J);if(jc(u,e),u=kc(u),!s)return u;var f;s=d[0];var V=a[0];d=d[1];a=a[1];s>V&&(f=s,s=V,V=f);var g=V-s,l=Math.abs(g-E)<N,m=l||N>g;if(!l&&d>a&&(f=d,d=a,a=f),m?l?0<d+a^u[1]<(Math.abs(u[0]-s)<N?d:a):d<=u[1]&&u[1]<=a:g>E^(s<=u[0]&&u[0]<=V))return f=rb(r,(-c+t)/J),jc(f,e),[u,kc(f)]}}function c(a,s){var r=e?d:E-d,p=0;return-r>a?p|=1:a>r&&(p|=2),-r>s?p|=4:s>r&&(p|=8),p}var p=
...
m)}else P.point(u[0],u[1])}function n(p,e){p>=d&&(r>=p&&e>=a&&c>=e)&&t.point(p,e)}function D(p,e){p=Math.max(-tb,Math.min(tb,p));e=Math.max(-tb,Math.min(tb,e));var u=p>=d&&r>=p&&e>=a&&c>=e;if(C&&z.push([p,e]),Z)B=p,A=e,G=u,Z=!1,u&&(t.lineStart(),t.point(p,e));else if(u&&sa)t.point(p,e);else{var f=[Q,M],V=[p,e],g=V[0]-f[0],P=V[1]-f[1],l=[0,1];(Math.abs(g)<N&&Math.abs(P)<N?d<=f[0]&&f[0]<=r&&a<=f[1]&&f[1]<=c:ub(d-f[0],g,l)&&(ub(f[0]-r,-g,l)&&ub(a-f[1],P,l)&&ub(f[1]-c,-P,l))&&(1>l[1]&&(V[0]=f[0]+l[1]*
g,V[1]=f[1]+l[1]*P),0<l[0]&&(f[0]+=l[0]*g,f[1]+=l[0]*P),!0))?(sa||(t.lineStart(),t.point(f[0],f[1])),t.point(V[0],V[1]),u||t.lineEnd()):(t.lineStart(),t.point(p,e))}Q=p;M=e;sa=u}var q,C,z,B,A,G,Q,M,sa,Z,ba=t,v=Ed(),y={point:n,lineStart:function(){y.point=D;C&&C.push(z=[]);Z=!0;sa=!1;Q=M=0/0},lineEnd:function(){q&&(D(B,A),G&&sa&&v.rejoin(),q.push(v.buffer()));y.point=n;sa&&t.lineEnd()},polygonStart:function(){t=v;q=[];C=[]},polygonEnd:function(){t=ba;(q=m.merge(q)).length?(t.polygonStart(),Ad(q,e,
...
a[1])}function e(){m=0/0;A.point=J;s.lineStart()}function J(a,c){var e=pb([a,c]),t=d(a,c);r(m,n,l,q,z,B,m=t[0],n=t[1],l=a,q=e[0],z=e[1],B=e[2],p,s);s.point(m,n)}function f(){A.point=c;s.lineEnd()}function g(){var d,a,c,V,F,ba;e();A.point=function(s,r){J(d=s,r);a=m;c=n;V=q;F=z;ba=B;A.point=J};A.lineEnd=function(){r(m,n,l,q,z,B,a,c,d,V,F,ba,p,s);A.lineEnd=f;f()}}var l,m,n,q,z,B,A={point:c,lineStart:e,lineEnd:f,polygonStart:function(){s.polygonStart();A.lineStart=g},polygonEnd:function(){s.polygonEnd();
...
a[c]};m.touches=function(d,a){return 2>arguments.length&&(a=R().touches),a?Aa(a).map(function(a){var c=H(d,a);return c.identifier=a.identifier,c}):[]};m.behavior.drag=function(){function d(){this.on("mousedown.drag",a).on("touchstart.drag",a)}function a(){function d(){var a=g.parentNode;return null!=q?m.touches(a).filter(function(d){return d.identifier===q})[0]:m.mouse(a)}function f(){l({type:"dragend"});D&&(v(),m.event.target===n&&U(I,"click"));I.on(null!=q?"touchmove.drag-"+q:"mousemove.drag",null).on(null!=
...
d.removeChild(this)})};W.data=function(d,c){function e(d,f){var r,p,g,u=d.length,l=f.length,J=Math.min(u,l),q=Array(l),V=Array(l),M=Array(u);if(c){var v=new a,y=new a,x=[];for(r=-1;++r<u;)J=c.call(p=d[r],p.__data__,r),v.has(J)?M[r]=p:v.set(J,p),x.push(J);for(r=-1;++r<l;)J=c.call(f,g=f[r],r),(p=v.get(J))?(q[r]=p,p.__data__=g):y.has(J)||(V[r]={__data__:g}),y.set(J,g),v.remove(J);for(r=-1;++r<u;)v.has(x[r])&&(M[r]=d[r])}else{for(r=-1;++r<J;)p=d[r],g=f[r],p?(p.__data__=g,q[r]=p):V[r]={__data__:g};for(;l>
r;++r)V[r]={__data__:f[r]};for(;u>r;++r)M[r]=d[r]}V.update=q;V.parentNode=q.parentNode=M.parentNode=d.parentNode;t.push(V);m.push(q);n.push(M)}var f,p,g=-1,l=this.length;if(!arguments.length){for(d=Array(l=(f=this[0]).length);++g<l;)(p=f[g])&&(d[g]=p.__data__);return d}var t=kd([]),m=K([]),n=K([]);if("function"==typeof d)for(;++g<l;)e(f=this[g],d.call(f,f.parentNode.__data__,g));else for(;++g<l;)e(f=this[g],d);return m.enter=function(){return t},m.exit=function(){return n},m};W.datum=function(d){return arguments.length?
...
Q[1])/M}).map(B.invert))}function g(d){f();m.event.preventDefault();d({type:"zoom",scale:M,translate:Q})}function l(){var d=this,c=Z.of(d,arguments),f=m.event.target,r=0,p=m.select(ha).on("mousemove.zoom",function(){r=1;e(m.mouse(d),t);g(c)}).on("mouseup.zoom",function(){r&&v();p.on("mousemove.zoom",null).on("mouseup.zoom",null);r&&m.event.target===f&&U(p,"click.zoom")}),t=a(m.mouse(d));ha.focus();v()}function t(){D||(D=a(m.mouse(this)));c(Math.pow(2,0.002*Kb())*M);e(m.mouse(this),D);g(Z.of(this,
...
isNaN(f)||0>=f?0:f}function c(e){var g=e.children;if(g&&g.length){var l,p=y(e),m=[],r=g.slice(),n=1/0,q="slice"===D?p.dx:"dice"===D?p.dy:"slice-dice"===D?e.depth&1?p.dy:p.dx:Math.min(p.dx,p.dy);a(r,p.dx*p.dy/e.value);for(m.area=0;0<(e=r.length);){m.push(e=r[e-1]);m.area+=e.area;var t;if(!(t="squarify"!==D)){e=q;l=void 0;var u=m.area;t=0;for(var v=1/0,x=-1,P=m.length;++x<P;)(l=m[x].area)&&(v>l&&(v=l),l>t&&(t=l));t=(l=(u*=u,e*=e,u?Math.max(e*t*I/u,u/(e*v*I)):1/0))<=n}t?(r.pop(),n=l):(m.area-=m.pop().area,
f(m,q,p,!1),q=Math.min(p.dx,p.dy),m.length=m.area=0,n=1/0)}m.length&&(f(m,q,p,!0),m.length=m.area=0);g.forEach(c)}}function e(c){var g=c.children;if(g&&g.length){var l=y(c),p=g.slice(),m=[];a(p,l.dx*l.dy/c.value);for(m.area=0;c=p.pop();)m.push(c),m.area+=c.area,null!=c.z&&(f(m,c.z?l.dx:l.dy,l,!p.length),m.length=m.area=0);g.forEach(e)}}function f(a,d,c,e){var g,l=-1,p=a.length,m=c.x,s=c.y,r=d?q(a.area/d):0;if(d==c.dx){for((e||r>c.dy)&&(r=c.dy);++l<p;)g=a[l],g.x=m,g.y=s,g.dy=r,m+=g.dx=Math.min(c.x+
c.dx-m,r?q(g.area/r):0);g.z=!0;g.dx+=c.x+c.dx-m;c.y+=r;c.dy-=r}else{for((e||r>c.dx)&&(r=c.dx);++l<p;)g=a[l],g.x=m,g.y=s,g.dx=r,s+=g.dy=Math.min(c.y+c.dy-s,r?q(g.area/r):0);g.z=!1;g.dy+=c.y+c.dy-s;c.x+=r;c.dx-=r}}function g(f){f=l||n(f);var p=f[0];return p.x=0,p.y=0,p.dx=v[0],p.dy=v[1],l&&n.revalue(p),a([p],p.dx*p.dy/p.value),(l?e:c)(p),S&&(l=f),f}var l,n=m.layout.hierarchy(),q=Math.round,v=[1,1],x=null,y=Ic,S=!1,D="squarify",I=0.5*(1+Math.sqrt(5));return g.size=function(a){return arguments.length?
...
1E-6),H=m.transition(S.exit()).style("opacity",1E-6).remove(),L=m.transition(S).style("opacity",1),R=Bb(e),d=d.selectAll(".domain").data([0]),d=(d.enter().append("path").attr("class","domain"),m.transition(d)),N=e.copy(),O=this.__chart__||N;this.__chart__=N;C.append("line");C.append("text");var K=C.select("line"),U=L.select("line"),z=S.select("text").text(z),S=C.select("text"),T=L.select("text");switch(f){case "bottom":a=Oe;B.attr("y2",l);A.attr("x2",0).attr("y2",l);K.attr("y2",g);S.attr("y",Math.max(g,
0)+q);U.attr("x2",0).attr("y2",g);T.attr("x",0).attr("y",Math.max(g,0)+q);z.attr("dy",".71em").style("text-anchor","middle");d.attr("d","M"+R[0]+","+n+"V0H"+R[1]+"V"+n);break;case "top":a=Oe;B.attr("y2",-l);A.attr("x2",0).attr("y2",-l);K.attr("y2",-g);S.attr("y",-(Math.max(g,0)+q));U.attr("x2",0).attr("y2",-g);T.attr("x",0).attr("y",-(Math.max(g,0)+q));z.attr("dy","0em").style("text-anchor","middle");d.attr("d","M"+R[0]+","+-n+"V0H"+R[1]+"V"+-n);break;case "left":a=Pe;B.attr("x2",-l);A.attr("x2",
-l).attr("y2",0);K.attr("x2",-g);S.attr("x",-(Math.max(g,0)+q));U.attr("x2",-g).attr("y2",0);T.attr("x",-(Math.max(g,0)+q)).attr("y",0);z.attr("dy",".32em").style("text-anchor","end");d.attr("d","M"+-n+","+R[0]+"H0V"+R[1]+"H"+-n);break;case "right":a=Pe,B.attr("x2",l),A.attr("x2",l).attr("y2",0),K.attr("x2",g),S.attr("x",Math.max(g,0)+q),U.attr("x2",g).attr("y2",0),T.attr("x",Math.max(g,0)+q).attr("y",0),z.attr("dy",".32em").style("text-anchor","start"),d.attr("d","M"+n+","+R[0]+"H0V"+R[1]+"H"+n)}if(e.ticks)C.call(a,
...
g[1],m=K[d],n=F[1][d]-F[0][d];return O&&(c-=m,g-=n+m),e=Math.max(c,Math.min(g,a[d])),O?f=(e+=m)+n:(z&&(m=Math.max(c,Math.min(g,2*z[d]-e))),e>m?(f=e,e=m):f=m),F[0][d]!==e||F[1][d]!==f?(l=null,F[0][d]=e,F[1][d]=f,!0):void 0}function C(){y();M.style("pointer-events","all").selectAll(".resize").style("display",a.empty()?"none":null);m.select("body").style("cursor",null);U.on("mousemove.brush",null).on("mouseup.brush",null).on("touchmove.brush",null).on("touchend.brush",null).on("keydown.brush",null).on("keyup.brush",
null);H({type:"brushend"});v()}var z,B,A=this,G=m.select(m.event.target),H=n.of(A,arguments),M=m.select(A),R=G.datum(),L=!/^(n|s)$/.test(R)&&q,N=!/^(e|w)$/.test(R)&&x,O=G.classed("extent"),K=p(),U=m.select(ha).on("mousemove.brush",y).on("mouseup.brush",C).on("touchmove.brush",y).on("touchend.brush",C).on("keydown.brush",function(){32==m.event.keyCode&&(O||(z=null,K[0]-=F[1][0],K[1]-=F[1][1],O=2),v())}).on("keyup.brush",function(){32==m.event.keyCode&&2==O&&(K[0]+=F[1][0],K[1]+=F[1][1],O=0,v())});
...
{y:1900,m:0,d:1,H:0,M:0,S:0,L:0};if(Gb(e,a,c,0)!=c.length)return null;"p"in e&&(e.H=e.H%12+12*e.p);c=new ia;return c.setFullYear(e.y,e.m,e.d),c.setHours(e.H,e.M,e.S,e.L),c},c.toString=function(){return a},c};var mf=Hb(kf),nf=Hb(lf),of=Hb(Yc),nh=Qe(Yc),pf=Hb(Zc),oh=Qe(Zc),mh={"-":"",_:" ","0":"0"},Rb={a:function(a){return lf[a.getDay()]},A:function(a){return kf[a.getDay()]},b:function(a){return Zc[a.getMonth()]},B:function(a){return Yc[a.getMonth()]},c:m.time.format("%a %b %e %X %Y"),d:function(a,
c){return la(a.getDate(),c,2)},e:function(a,c){return la(a.getDate(),c,2)},H:function(a,c){return la(a.getHours(),c,2)},I:function(a,c){return la(a.getHours()%12||12,c,2)},j:function(a,c){return la(1+m.time.dayOfYear(a),c,3)},L:function(a,c){return la(a.getMilliseconds(),c,3)},m:function(a,c){return la(a.getMonth()+1,c,2)},M:function(a,c){return la(a.getMinutes(),c,2)},p:function(a){return 12<=a.getHours()?"PM":"AM"},S:function(a,c){return la(a.getSeconds(),c,2)},U:function(a,c){return la(m.time.sundayOfYear(a),
c,2)},w:function(a){return a.getDay()},W:function(a,c){return la(m.time.mondayOfYear(a),c,2)},x:m.time.format("%m/%d/%Y"),X:m.time.format("%H:%M:%S"),y:function(a,c){return la(a.getFullYear()%100,c,2)},Y:function(a,c){return la(a.getFullYear()%1E4,c,4)},Z:function(a){var c=a.getTimezoneOffset();a=0<c?"-":"+";var e=~~(Math.abs(c)/60),c=Math.abs(c)%60;return a+la(e,"0",2)+la(c,"0",2)},"%":function(){return"%"}},Kg={a:function(a,c,e){nf.lastIndex=0;return(a=nf.exec(c.substring(e)))?e+a[0].length:-1},
...
m.time.format.iso=Date.prototype.toISOString&&+new Date("2000-01-01T00:00:00.000Z")?Nc:qf;Nc.parse=function(a){a=new Date(a);return isNaN(a)?null:a};Nc.toString=qf.toString;m.time.second=Ka(function(a){return new ia(1E3*Math.floor(a/1E3))},function(a,c){a.setTime(a.getTime()+1E3*Math.floor(c))},function(a){return a.getSeconds()});m.time.seconds=m.time.second.range;m.time.seconds.utc=m.time.second.utc.range;m.time.minute=Ka(function(a){return new ia(6E4*Math.floor(a/6E4))},function(a,c){a.setTime(a.getTime()+
...
3],[m.time.year,1]],qh=[[m.time.format("%Y"),Xa],[m.time.format("%B"),function(a){return a.getMonth()}],[m.time.format("%b %d"),function(a){return 1!=a.getDate()}],[m.time.format("%a %d"),function(a){return a.getDay()&&1!=a.getDate()}],[m.time.format("%I %p"),function(a){return a.getHours()}],[m.time.format("%I:%M"),function(a){return a.getMinutes()}],[m.time.format(":%S"),function(a){return a.getSeconds()}],[m.time.format(".%L"),function(a){return a.getMilliseconds()}]],rf=m.scale.linear(),rh=Te(qh);
$c.year=function(a,c){return rf.domain(a.map(Lg)).ticks(c).map(Qc)};m.time.scale=function(){return Oc(m.scale.linear(),$c,rh)};var sf=$c.map(function(a){return[a[0].utc,a[1]]}),sh=[[m.time.format.utc("%Y"),Xa],[m.time.format.utc("%B"),function(a){return a.getUTCMonth()}],[m.time.format.utc("%b %d"),function(a){return 1!=a.getUTCDate()}],[m.time.format.utc("%a %d"),function(a){return a.getUTCDay()&&1!=a.getUTCDate()}],[m.time.format.utc("%I %p"),function(a){return a.getUTCHours()}],[m.time.format.utc("%I:%M"),
...
c).response(Pg)},m.xml=function(){return m.xhr.apply(m,arguments).response(Qg)},m}();var w=$("#test-nav").width(),h=800,i=0,barHeight=20,barWidth=0.8*w,duration=400,root,tree=d3.layout.tree().size([h,100]),diagonal=d3.svg.diagonal().projection(function(f){return[f.y,f.x]});
...
$(document).ready(function(){updateShotNav();$("#main").scrollView();autoPollSummaryDB();loadCookie();$(".h1ds-pagelet").each(function(){populatePagelet($(this),window.location.toString())})});
