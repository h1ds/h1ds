...
function loadWorksheet(){var m=[{plot_coords:[0,0,10,10],modifiers:{},data:[{url:window.location.toString(),url_parser:function(m,b){var d=getRawUri(m,b.width);b.hasOwnProperty("min")&&d.updateOrAddBaseFilter("dim_range",{min:b.min,max:b.max});var g;$.ajax({url:d.renderExtraUri(),dataType:"json",async:!1}).done(function(b){g=b.data});return g}}]}],r=dataviewer.create("div.data.worksheet");r.data(m);r.draw()}$(document).ready(function(){$("#main").scrollView();autoPollSummaryDB();loadCookie();loadWorksheet()});
