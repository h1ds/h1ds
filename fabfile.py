...
def deploy():
    ...
    if env.environment == 'development':
        ...
        conf_template = open('%s/h1ds/conf/apache/h1ds_staging.conf.template' %venv_dir)
        conf_text = conf_template.read()
        conf_template.close()
        conf_text = conf_text.replace("__venv_dir__", venv_dir)
        conf_text = "# DO NOT EDIT THIS FILE DIRECTLY, MAKE CHANGES TO THE TEMPLATE FILE AND RUN fab staging deploy\n"+conf_text
        conf_file = open('%s/h1ds/conf/apache/h1ds_staging.conf' %venv_dir, 'w')
        ...
        if not os.path.exists(h1ds_apache_conf):
            sudo("ln -s %s/h1ds/conf/apache/h1ds_staging.conf %s" %(venv_dir, h1ds_apache_conf))
            sudo("a2ensite h1ds")
